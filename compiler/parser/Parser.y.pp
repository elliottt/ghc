...
kconstrs :: { Located [LConDecl RdrName] }
        : maybe_docnext '=' kconstrs1
                { L (comb2 $2 $3) (addConDocs (reverse (unLoc $3)) $1) }

kconstrs1 :: { Located [LConDecl RdrName] }
        : kconstrs1 maybe_docnext '|' maybe_docprev kconstr
                { LL (addConDoc $5 $2 : addConDocFirst (unLoc $1) $4) }
        | kconstr
                { L1 [$1] }

kconstr :: { LConDecl RdrName }
        : maybe_docnext kconstr_stuff maybe_docprev
                { let (con,details) = unLoc $2 in
                  addConDoc (L (getLoc $2) (mkTyConDecl con details)) ($1 `mplus` $3)
                }

kconstr_stuff :: { Located (Located RdrName, HsConDeclDetails RdrName) }
